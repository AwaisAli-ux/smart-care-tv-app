...
class MainActivity : FlutterActivity() {
    ...
    private val CHANNEL             = "com.smartcaretv.app/audio"
    private val DEVICE_INFO_CHANNEL  = "com.smartcaretv.app/device_info"
    private val TV_KEYS_CHANNEL      = "com.smartcaretv.app/tv_keys"
    ...
    /** Ensures volume is at max when the app starts (if it was muted or very low). */
    private fun initVolumeOnce() {
        try {
            val am = audioManager ?: return
            val maxVol = am.getStreamMaxVolume(AudioManager.STREAM_MUSIC)
            val curVol = am.getStreamVolume(AudioManager.STREAM_MUSIC)
            // If volume is 0 or below 30%, restore to 100% so streams are always audible
            if (curVol < (maxVol * 0.3).toInt().coerceAtLeast(1)) {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "requestAudioFocus" -> {
                        // AudioManager.requestAudioFocus() is a synchronous Binder call into
                        // AudioService. On phones with heavy OEM audio stacks (Samsung/Xiaomi/
                        // OPPO Dolby/Atmos/spatial-audio processing, active calls, Bluetooth
                        // routing) this can stall for seconds — or, as observed on a stock
                        // Pixel, simply never return at all. Previously this handler replied to
                        // Flutter only AFTER requestAudioFocus() finished (Thread { ...;
                        // handler.post { result.success(true) } }), so a stuck Binder call meant
                        // the reply never fired and Flutter's `await` on the Dart side blocked
                        // forever — which is exactly what produced the "Waited 5002ms for
                        // FocusEvent" ANR the instant a channel was tapped: MethodChannel results
                        // are asserted single-reply and thread-confined, so there is no way to
                        // route around a call whose completion the reply itself depends on.
                        // Fix: reply immediately via the main handler, and let requestAudioFocus()
                        // run fully fire-and-forget in the background — the Dart side never needs to
                        // know when (or if) it actually completes.
                        handler.post {
                            try {
                                result.success(true)
                            } catch (e: Exception) {
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, DEVICE_INFO_CHANNEL)
            .setMethodCallHandler { call, result ->
                when (call.method) {
                    "getDeviceInfo" -> {
                        val hasHdr = supportsHdr()
                        Thread {
                            try {
                                val info = getDeviceInfo(hasHdr)
                                handler.post {
                                    result.success(info)
                                }
                            } catch (e: Exception) {
    ...
    private fun getDeviceInfo(supportsHdr: Boolean): Map<String, Any> {
        ...
        // Build.SERIAL is deprecated in API 26+ but still readable on many TV boxes.
        // We combine ANDROID_ID (always reliable) as the primary hardware identifier.
        val hwSerial = try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                // On API 26+ requires READ_PHONE_STATE; falls back gracefully
                Build.getSerial()
            } else {
                @Suppress("DEPRECATION")
                Build.SERIAL
        ...
        // Scan all codecs exactly once to prevent main thread blocking (ANR)
        ...
        try {
            ...
            for (info in list.codecInfos) {
                if (!info.isEncoder) {
                    try {
                        codecTypes.addAll(info.supportedTypes)
                    } catch (e: Exception) {
        ...
        val videoCodecs = codecTypes.filter { it.startsWith("video/") }.sorted()

        return mapOf(
            "deviceId"        to androidId,
            "hwSerial"        to hwSerial,
            "isTv"            to isTelevisionDevice(),
            "brand"           to Build.BRAND,           // original case: "google", "Samsung", etc.
            "manufacturer"    to Build.MANUFACTURER,    // original case: "Google", "samsung", etc.
            "model"           to Build.MODEL,
            "device"          to Build.DEVICE,
            "osVersion"       to Build.VERSION.RELEASE,
            "sdkInt"          to Build.VERSION.SDK_INT,
            "totalRamMb"      to totalRamMb,
            "supportsHevc"    to codecTypes.contains("video/hevc"),
            "supportsHdr"     to supportsHdr,
            "supportsAv1"     to codecTypes.contains("video/av01"),
            "supportsVp9"     to codecTypes.contains("video/x-vnd.on2.vp9"),
            "supportsH264"    to codecTypes.contains("video/avc"),
            "supportsAac"     to codecTypes.contains("audio/mp4a-latm"),
            "supportsAc3"     to codecTypes.contains("audio/ac3"),
            "supportsEac3"    to codecTypes.contains("audio/eac3"),
            "supportedVideoCodecs" to videoCodecs
        )
    }

    /**
     * True only for actual TV hardware (Android TV / Google TV certified boxes,
     * Fire TV, or devices exposing the leanback/television system feature).
     * Brand-name string matching alone (used downstream for UA/codec quirks)
     * mis-detects real Samsung/Sony/Xiaomi/TCL PHONES as their TV counterparts,
     * since those manufacturers sell both — this flag lets DeviceProfileService
     * gate all TV-only branching so phones always get safe generic-mobile
     * defaults instead of Smart-TV specific UA strings / mpv quirks.
     */
    private fun isTelevisionDevice(): Boolean {
        return try {
            ...
            val hasTelevisionFeature = packageManager.hasSystemFeature(android.content.pm.PackageManager.FEATURE_TELEVISION)
            isUiModeTv || hasLeanback || hasTelevisionFeature
        } catch (e: Exception) {
            false
        }
    }
    ...
    private fun getVolumePercent(): Int {
        return try {
            ...
            if (max > 0) ((cur.toFloat() / max) * 100).toInt() else 100
        } catch (e: Exception) {
            100
        }
    }
    ...
    private fun requestAudioFocus() {
        Thread {
            try {
                val am = audioManager ?: return@Thread
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
